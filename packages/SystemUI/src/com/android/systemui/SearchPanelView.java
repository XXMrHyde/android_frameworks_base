       ...
       final Runnable SetLongPress = new Runnable () {
            public void run() {
                if (!mSearchPanelLock) {
                    ...
                    if (!DkActions.isActionKeyEvent(mLongList.get(mTarget))) {
                        performHapticFeedback(HapticFeedbackConstants.LONG_PRESS);
                    }
                    sendAccessibilityEvent(AccessibilityEvent.TYPE_VIEW_LONG_CLICKED);
                    DkActions.processAction(mContext, mLongList.get(mTarget), true);
        ...
        public void onTrigger(View v, final int target) {
            ...
            mTarget = target;
            if (!mLongPress) {
                if (!DkActions.isActionKeyEvent(mIntentList.get(target))) {
                ...
                DkActions.processAction(mContext, mIntentList.get(target), false);
                mHandler.removeCallbacks(SetLongPress);
            }
        }
            ...
            File iconFile = new File(Uri.parse(customIconUri).getPath());
                try {
                    ...
                    if (iconFile.exists()) {
                        ...
                        customIcon = ImageHelper.resize(mContext, customIcon, 60);
                    } else {
                        customIcon = new BitmapDrawable(getResources(),
