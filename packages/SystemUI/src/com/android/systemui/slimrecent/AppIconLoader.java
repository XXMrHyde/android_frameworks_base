...
public class AppIconLoader {
    ...
    private static Drawable getAppIcon(ResolveInfo info, Context context, float scaleFactor) {
        ...
        return getResizedBitmapDrawable(getFullResIcon(context, info, pm), context, scaleFactor);
     ...
     */
    private static Drawable getResizedBitmapDrawable(Drawable source, Context context, float scaleFactor) {
        if (source == null) {
            return null;
        }
    ...
    private static class BitmapDownloaderTask extends AsyncTask<ResolveInfo, Void, Drawable> {
        ...
        protected void onPostExecute(Drawable drawable) {
            ...
            if (rImageViewReference != null) {
                ...
                if (drawable != null && context != null && drawable instanceof BitmapDrawable) {
                    // Put our drawable intu LRU cache for later use.
                    CacheController.getInstance(context)
                            .addBitmapDrawableToMemoryCache(mLRUCacheKey, (BitmapDrawable) drawable);
                }
            }
        }
    }
