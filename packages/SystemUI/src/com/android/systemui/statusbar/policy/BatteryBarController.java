...
public class BatteryBarController extends LinearLayout {
    ...
    public void updateSettings() {
        ...
        boolean useThemeDefaultColors;
        final int defaultFrameColor = res.getColor(R.color.batterymeter_frame_color);
        final int lowColor = res.getColor(R.color.batterymeter_low);
        final int defaultFullColor = res.getColor(R.color.batterymeter_full);
        ...
        int fullColor = Settings.System.getIntForUser(resolver,
                Settings.System.STATUS_BAR_BATTERY_BAR_COLOR, defaultFullColor,
                UserHandle.USER_CURRENT);

        if (fullColor == 0xffffffff) {
            useThemeDefaultColors = true;
        } else {
            useThemeDefaultColors = false;
        }
        if (useThemeDefaultColors) {
            ((View)this).setBackgroundColor(defaultFrameColor);
        } else {
